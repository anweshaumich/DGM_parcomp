function [Res1,Res2, del_est, thet_est] = langevin_dat_gaussian(X,gam,sam,step_in,c,block,Niter,burn,gam1,rho_in,rho_fixed, tow_in,compact_in,mc_rate,inc,nr_of_cores)
diary my_data.out;
fprintf('start')
%clear all;
...
%parameter for Update tow
    ...
    %%Update theta
        ...
        s = find(dd>0);
        if(isempty(s)==0)
             lo = likehdder(theta_old,Rg,h_matj,H_mat);
            g_old = lo + rho*sign(theta_old.*dd)  + (theta_old.*(1-dd))/gam;
            G_old = g_old*(c/max(c,norm(g_old)));
            h_old = likhd(theta_old,Rg,h_matj,H_mat) + rho*sum(abs(theta_old.*dd)) + (.5/gam)*norm(theta_old.*(1-dd))^2;
        ...
        for i = s
            ...
            ln = likehdder(theta_new,Rg,h_matj,H_mat);
            g_new = ln + rho*sign(theta_new.*dd)  + (theta_new.*(1-dd))/gam;
            G_new = g_new*(c/max(c,norm(g_new)));
            h_new = likhd(theta_new,Rg,h_matj,H_mat) + rho*sum(abs(theta_new.*dd)) + (.5/gam)*norm(theta_new.*(1-dd))^2;
  ...
  %normal
        ...
        if do >0
           ...
           %form blocks
            ...
            [ec,w] = wvm(theta_old.*dd,h_matj,H_mat,Rg);
            
            
            cmat = H_mat - ec;  
            weight = sqrt(sum(cmat.^2,2)).*w;  weight = weight/sum(weight);
            idx = datasample(1:m*n,sam,'Replace',false, 'Weights',weight); lidx = length(idx);
            cmat = cmat(idx,:); W  =  sparse(1:lidx,1:lidx,w(idx),lidx,lidx);
            ...
            %block update
            for s = 1:cl
                act = set(y==s);  sact = length(act);
                cact = cmat(:,act);% cinact = cmat(:,inact);
                ind = ones(1,p);ind(act) = 0;
                ldr = likehdder(theta_old.*ind,Rg,h_matj,H_mat);
                Sig = eye(sact) + (gam)*(cact'*W*cact);
                chl = chol(Sig); dim = size(Sig,1); chl_in = chl\eye(dim); Sig_in = chl_in*chl_in';
                ...
                theta_new = theta_old;
                theta_new(act) = un;
                k3 = -likhd(theta_new,Rg,h_matj,H_mat) - (.5/gam)*norm(theta_new.*(1-dd))^2 + likhd(theta_old,Rg,h_matj,H_mat) + (.5/gam)*norm(theta_old.*(1-dd))^2;
