...
%     d_init = dd;
    ...
    %%Update theta
        ...
        s = find(dd>0);
        if(isempty(s)==0)
             lo = likehdder(theta_old,dd,Rg,X,j);
             g_old = lo + (theta_old.*dd)/rho  + (theta_old.*(1-dd))/gam;
             G_old = g_old*(c/max(c,norm(g_old)));
             h_old = likhd(theta_old,dd,Rg,X,j) + (.5/rho)*norm(theta_old.*dd)^2 + (.5/gam)*norm(theta_old.*(1-dd))^2;
        ...
        for i = s
            ...
            ln = likehdder(theta_new,dd,Rg,X,j);
            g_new = ln + (theta_new.*dd)/rho  + (theta_new.*(1-dd))/gam;
            G_new = g_new*(c/max(c,norm(g_new)));
            h_new = likhd(theta_new,dd,Rg,X,j) + (.5/rho)*norm(theta_new.*dd)^2 + (.5/gam)*norm(theta_new.*(1-dd))^2;
  ...
  %normal
     ...
     h_old = likhd(theta_old,dd,Rg,X,j) + sum(dd)*log(sqrt(rho)) + sum(1-dd)*log(sqrt(gam)) ...
    ...
    id = find(ddprop~=dd);
     for i = id
            ...
            ddn = dd;
            ddn(i) = ddprop(i);
            h_new = likhd(theta_old,ddn,Rg,X,j)+ sum(ddn)*log(sqrt(rho))+ sum(1-ddn)*log(sqrt(gam)) + ...
